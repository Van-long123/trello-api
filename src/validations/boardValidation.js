...
const update = async (req, res, next) => {
  ...
  try {
    await correctCondition.validateAsync(req.body, {
      abortEarly:false,
      // Nó cho phép các field KHÔNG được khai báo trong schema vẫn được giữ lại và không báo lỗi.
      allowUnknown: true
    })
    next()
  } catch (error) {
...
const moveCartToDifferentColumn = async (req, res, next) => {
  const correctCondition = Joi.object({
    ...
    targetBoardId: Joi.string().pattern(OBJECT_ID_RULE).message(OBJECT_ID_RULE_MESSAGE),
